...
func (db *DB) GetAllUsers(nodeID uint) ([]UserModel, error) {
	...
	err := db.DB.Select("id, vmess_id, username, port").Where("enable = 1 AND u + d < transfer_enable AND level >= ? AND node_group = ?", node.NodeLevel, node.NodeGroup).Find(&users).Error
	return users, err
}
